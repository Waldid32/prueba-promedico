...
export async function GET() {
    try {
        const result = await pool.query(`
            SELECT clients.id, clients.nombres, clients.cedula, clients.celular, clients.direccion, ciudad.ciudad, dep.departamento, agentes.nombres AS "agente" FROM clients
            INNER JOIN ciudad ON clients.ciudad = ciudad.id
            INNER JOIN departamento dep ON dep.id = ciudad.id_departamento
            INNER JOIN agentes ON agentes.id = clients.id_agent
        `)
